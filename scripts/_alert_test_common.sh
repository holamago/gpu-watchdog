...
TRAINING_PATTERN="gpu-watchdog-fake-train-$RANDOM-$$"
...
start_fake_training() {
  python3 -c 'import time; time.sleep(600)' "$TRAINING_PATTERN" &
  TRAIN_PID="$!"
  sleep 0.3
  echo "Started fake training process: pid=$TRAIN_PID pattern=$TRAINING_PATTERN"
}
